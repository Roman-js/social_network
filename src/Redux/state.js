...
export let addDialog = () =>{
   let newDialog = {id: 4, text: state.dialogsPage.newPostText};
   state.dialogsPage.messages.push(newDialog);
    rerenderEntireTree(state);
};
export let updateComment = (currentDialog) =>{
    state.dialogsPage.newPostText = currentDialog;
    rerenderEntireTree(state);

}

export let addPost = (postMessage) =>{
    let newPost = {id: 4, message: state.profilePage.newPostText, likes: 0};
    let copyPosts = [...state.profilePage.posts, newPost];
    state.profilePage.posts.push(newPost)
    rerenderEntireTree(state);
};

export let updatePost =(currentPost) =>{
    state.profilePage.newPostText=currentPost;
    rerenderEntireTree(state);
}
